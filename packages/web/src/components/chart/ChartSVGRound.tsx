...
const PALACE_RADIUS = 195;
const GUIDE_RADIUS = PALACE_RADIUS + 18;
const OUTER_RING_RADIUS = PALACE_RADIUS + CELL_SIZE / 2 + 12;
...
function centerByBranch(branch: EarthlyBranch): Point {
  const index = 地支.indexOf(branch);
  if (index < 0) return { x: CENTER, y: CENTER };
  // Keep "午" near top and move clockwise by branch order.
  const angle = ((index - 6) / 12) * (Math.PI * 2) - Math.PI / 2;
  return {
    x: CENTER + PALACE_RADIUS * Math.cos(angle),
    y: CENTER + PALACE_RADIUS * Math.sin(angle),
  };
}
...
export default function ChartSVGRound() {
  ...
  const branchCenters = useMemo(() => {
    ...
    for (const branch of 地支) {
      map[branch] = centerByBranch(branch);
    }
    return map;
  }, []);
  ...
  return (
    ...
    >
      <circle
        ...
        fill="none"
        stroke="currentColor"
        className="text-zinc-300 dark:text-sky-700"
        strokeWidth={1.4}
      />
      <circle
        cx={CENTER}
      ...
      {地支.map((branch) => {
        const point = branchCenters[branch];
        return (
          <line
            key={`guide-${branch}`}
            x1={CENTER}
      ...
      <g>
        <circle cx={CENTER} cy={CENTER} r={80} fill="currentColor" className="text-slate-900" />
      </g>
      ...
      {地支.map((branch) => {
        ...
        return (
          <PalaceCell
            ...
            palace={palace}
            x={point.x - CELL_SIZE / 2}
            y={point.y - CELL_SIZE / 2}
            size={CELL_SIZE}
            stars={visibleStars}
            isMing={isMing}
            isBody={isBody}
      ...
      <g>
        <foreignObject x={CENTER - 72} y={CENTER - 72} width={144} height={144}>
          <div className="h-full w-full rounded-full p-3 flex flex-col items-center justify-center text-center">
            <div className="text-xs font-semibold surface-value">Ziwei</div>
            <div className="mt-1 text-[11px] surface-help">{formatSolarDate(chart)}</div>
            <div className="text-[11px] surface-help">{formatLunarDate(chart)}</div>
            <div className="mt-1 text-[10px] surface-help">
              {chart.date.ganzhi.year} / {chart.date.ganzhi.month} / {chart.date.ganzhi.day}
            </div>
            <div className="mt-1 text-[9px] surface-help">
            ...
            {sanheSummary ? <div className="mt-1 text-[9px] surface-help">三方四正：{sanheSummary}</div> : null}
            <div className="mt-1 text-[9px] surface-help">
              年四化：{mutagenSummary(cycleSnapshot?.yearly?.yearlyMutagens ?? null)}
            </div>
            <div className="mt-1 text-[9px] surface-help">
              宫干飞化：点击宫位查看 · Shift 多选
            </div>
          </div>
        </foreignObject>
      </g>
